...
setMethod(
  ...
  function(object, param, ref_index = NULL, ref_sample = NULL, num_pools = NULL) {
    ...
    names(files) <- object@files
    par_idx <- .gen_split_indexes(length(object@files), ceiling(length(query_indexes) / num_pools))
...
setMethod(
  ...
  function(object, param, ref_index = NULL, ref_sample = NULL) {
    ...
    par_idx <- .gen_split_indexes(length(query_indexes), object@experiment@BPPARAM$workers)
