...
#' parameters for querying TIMS data
#' @export
QueryTimsDataSegmentParam <- function(
  opentims_thread = 2,
  time_limit_table = 'time_limit_table.csv',
  pool_size = 2000L,
  overlaps = 2L,
  rerun = FALSE
) {
  new("QueryTimsDataSegmentParam",
      opentims_thread = opentims_thread,
      time_limit_table = time_limit_table,
      pool_size = pool_size,
      overlaps = overlaps,
      rerun = rerun
  )
...
#' Trim data according to the provided time limit table
#'
#' @param time_limit_table \code{character} path to the time limit table
#' @param rerun \code{logical} if rerun the process
#'
#' @export
TrimSCTimsDataParam <- function(
  time_limit_table = 'time_limit_table.csv',
  rerun = FALSE
) {
  return(new("TrimSCTimsDataParam",
             time_limit_table = time_limit_table,
             rerun = rerun))
}


#' Find single cell event according to the provided single cell marker
#'
#' @param marker_mz \code{numeric} m/z of the single-cell event marker
#' @param marker_ccs \code{numeric} ccs of the single-cell event marker
#' @param intensity_abs_threshold_upper \code{numeric} relative intensity upper threshold of the single-cell event marker
#' @param intensity_abs_threshold_lower \code{numeric} relative intensity lower threshold of the single-cell event marker
#'
#' @export
DiscoverSCEventsParam <- function(
  marker_mz = 760.5851,
  marker_mobility = 1.355,
  mz_tolerance_ppm = 20,
  ...
  return(new("DiscoverSCEventsParam",
             marker_mz = marker_mz,
             marker_mobility = marker_mobility,
...
## ExtractSCIMDataParam
#' parameters for extracting EIM in single-cell events
#' @export
ExtractSCIMDataParam <- function(
  mz_tol = 20,
  mobility_tol = 0.05,
  min_intensity = 0,
  min_points = 10,
  ...
  return(new("ExtractSCIMDataParam",
             mz_tol = mz_tol,
             mobility_tol = mobility_tol,
             min_intensity = min_intensity,
             min_points = min_points,
...
#' @export
UniteCellSuperpositionFrameParam <- function(
  mz_tolerance_combine = 20,
  cell_number = NULL,
  pool_size = 40L,
  rerun = FALSE
) {
  new("UniteCellSuperpositionFrameParam",
      mz_tolerance_combine = mz_tolerance_combine,
...
#' @export
SearchPeakTargetParam <- function(
  mz_tol_ppm = 20,
  n_skip = 1,
  peak_target_length = 15,
  rerun = FALSE
) {
  new("SearchPeakTargetParam",
      mz_tol_ppm = mz_tol_ppm,
      n_skip = n_skip,
      peak_target_length = peak_target_length,
      rerun = rerun)
}


#' @export
DetectEIMPeaksParam <- function(
  smooth_window = 10,
  ...
  new("DetectEIMPeaksParam",
      smooth_window = smooth_window,
      peak_span_eim_detection = peak_span_eim_detection,
      peak_span_eim_integration = peak_span_eim_integration,
      signal_sd_threshold = signal_sd_threshold,
...
#' @export
ExtractIndividualCellParam <- function(
  mz_tol_ppm = 20,
  mobility_tol = 0.04,
  ...
  new("ExtractIndividualCellParam",
      mz_tol_ppm = mz_tol_ppm,
      mobility_tol = mobility_tol,
      minimal_cell_number = minimal_cell_number,
      pool_size = pool_size,
      rerun = rerun)
}
...
#' #' @export
#' GenerateSCFeatureParam <- function(
#'     minSamples = 1,
#'     minFraction = 0.1,
#'     rerun = FALSE
#' ) {
#'   new("GenerateSCFeatureParam",
