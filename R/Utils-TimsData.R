...
.get_peak_name2 <- function(info) {
  info <- round(info)
  res <- apply(mapply(paste0, c("M", "C"), info), 1, paste0, collapse = "")
  res <- .make_unique_names(res)
  return(res)
}
...
.get_eims3 <- function(frame_list, scan_mobility, mz, mz_tol, mobility, mobility_tol) {
  ...
  names(temp) <- target_scans
  eims <- lapply(frame_list, function(frame) {
    eim <- .get_frame_data3(frame, target_scans, temp, mz, mz_tol)
  })
  if(all(eims[[1]] == 0)){
    return(NULL)
  }
  ...
  # res <- data.frame(do.call(cbind, eims[[1]]), check.names = FALSE)
  res <- eims[[1]]
  res$k0 <- target_mobility
  attr(res, "mz") <- attributes(eims[[1]])$mz
  return(res)
}
...
.get_frame_data3 <- function(frame, target_scans, temp, mz, mz_tol) {
  ...
  # temp[as.character(frame$scan)] <- frame$intensity
  new_temp <- data.frame(new_scan = names(temp),
                         intensity = temp)
  idx <- fastmatch::fmatch(new_temp$new_scan, frame$scan)
  new_temp$mz <- frame$mz[idx]
  new_temp$intensity <- frame$intensity[idx]
  new_temp$num_ion <- frame$num_ion[idx]
  new_temp$intensity[which(is.na(new_temp$intensity))] <- 0
  temp <- new_temp
  attr(temp, "mz") <- .get_weighted_mz(frame$mz, frame$intensity)
  return(temp)
}
...
#   new_temp$intensity[which(is.na(new_temp$intensity))] <- 0
#   temp <- new_temp
#   temp$mz <- .get_weighted_mz(frame$mz, frame$intensity)
#   return(temp)
# }



.get_eims4 <- function(scan_list, mz, mz_tol) {
  eims <- lapply(scan_list, function(scan) {
...
.get_frame_data4 <- function(scan, mz, mz_tol) {
  mz_diff <- abs(scan$mz - mz)
  cad <- which.min(mz_diff)
  if (mz_diff[cad] <= mz_tol) {
    return(scan[cad, , drop = FALSE])
  }
  # is_keep <- which(mz_diff <= mz_tol)
  # scan <- scan[is_keep, , drop = FALSE]
